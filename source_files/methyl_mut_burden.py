...
class methylomeMutationalBurden:
    ...
    def compare_all_pairs(self):
        ...
        for dset in self.all_methyl_age_df_t['dataset'].unique():
            for gender in ['FEMALE', 'MALE']:
                ...
                for pair in pairs:
                    compare_dict[pair] = (
                        methylome_df.loc[pair[0], :] 
                        - methylome_df.loc[pair[1], :]
                        ).abs().sum()
    ...
    def _observed_methyl_change(self, sample, comparison_samples):
        ...
        methylome_diffs = comp_samples_df.loc[:, cpg_cols].subtract(
            self.all_methyl_age_df_t.loc[sample, cpg_cols]
            )
    ...
    def compare_pairs(self, num_samples = -1, same_age = True, datasets: list = []):
        """
        ...
        for dset in datasets:
            ...
            # get a list of valid samples from this dataset to choose from
            # must have at least 1 ct mutation and methylation data
            samples_w_mut_and_methyl = list(
                set(dset_methyl_age_df_t.index)
                & set(self.ct_mut_counts_df.index)
                )
            #for sample in samples_w_mut_and_methyl:
            for sample in track(
                samples_w_mut_and_methyl, 
                total=len(samples_w_mut_and_methyl),
                description="Comparing pairs"
                ):
                ...
                if same_age:
                    same_age_dset_samples = dset_methyl_age_df_t[
                    ...
                    comparison_samples = list(
                        set(same_age_dset_samples.index.to_list()) 
                        & set(samples_w_mut_and_methyl)
                        )
                    if len(comparison_samples) == 0:
                        continue
                else:
                    comparison_samples = list(
                        set(dset_methyl_age_df_t.loc[dset_methyl_age_df_t['gender'] == gender].index.to_list())
                        & set(samples_w_mut_and_methyl)
                        )
                    if len(comparison_samples) == 0:
                        continue
