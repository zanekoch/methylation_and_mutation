...
_, all_methyl_age_gender_df_t = utils.add_ages_to_mut_and_methyl(all_mut_df, all_meta_df, all_methyl_df_t)

mbb = methyl_mut_burden.methylomeMutationalBurden(all_mut_df, all_methyl_age_gender_df_t, age_bin_size = 5)

pair_comp_df = mbb.compare_all_pairs()

pair_comp_df.to_parquet(os.path.join(out_dir, 'pair_comp_all_diffAges.parquet'))

corr_df = pair_comp_df.corr(method='spearman')
corr_df.to_parquet(os.path.join(out_dir, 'pair_comp_corr_spearman_all_diffAges.parquet'))